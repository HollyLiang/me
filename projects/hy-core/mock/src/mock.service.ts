...
import { MOCK_DEFULAT_CONFIG, MockConfig, MOCK_CONFIG } from './mock.config';
...
export class MockService implements OnDestroy {
  ...
  constructor(@Optional() @Inject(MOCK_CONFIG) mockConfig?: MockConfig) {
    this.config = MOCK_DEFULAT_CONFIG;
    deepMerge(this.config, mockConfig);
    this.applyMock();
